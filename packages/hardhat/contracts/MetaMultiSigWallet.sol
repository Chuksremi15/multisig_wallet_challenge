...
contract MetaMultiSigWallet {
    ...
    event TransferFunds(address indexed to, uint amount, uint balance);
    ...
    function transferFunds(
        ...
        require(amount > 0, "transferFunds: no amount");

        (bool success, ) = to.call{value: amount}("");

        require(success, "transferFunds: tx failed");

        emit TransferFunds(to, amount, address(this).balance);
    }
